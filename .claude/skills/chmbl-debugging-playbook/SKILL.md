...
| FSM stats shift when tunables change unexpectedly | `uv run tools/trc_viz.py logger/40mph_drive_cycle.trc --headless-check --decel-on-mphps 3.0` (every `BrakeTunables` field is a `--kebab-case` flag) | You (or a slider) changed a tunable; accel + FSM are recomputed live whenever one changes, including `speed_smooth_ms` | Baseline numbers and tunable semantics live in `chmbl-diagnostics-and-tooling` / docs/firmware.md |
| Brake light strobes / blinks in stop-and-go traffic (real ride, or transition/blip counts explode in replay) | `uv run .claude/skills/chmbl-diagnostics-and-tooling/scripts/fsm_metrics.py logger/40mph_drive_cycle.trc` (transition + sub-0.5 s blip counts) | FSM chatter around the decel/stop thresholds — quantized wheel speed oscillates across a single trigger line. The designed defenses are hysteresis (STOPPED entry/exit), the decel-on debounce, and the min-dwell floor | This is DE-09 territory: the hysteresis/debounce evidence and the fix path are owned by `chmbl-de09-campaign` (Phase 3/4); the choose-hysteresis-vs-rate-limiting method is `chmbl-proof-and-analysis-toolkit` |
